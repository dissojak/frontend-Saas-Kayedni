...
import {
  fetchBookingsForBusiness,
  fetchStaffByBusinessId,
  fetchServicesByBusinessId,
} from "../../../app/(pages)/(business)/actions/backend";
...
import { fetchProfile } from "@global/lib/api/profile.api";
...
export default function BusinessDashboard() {
  ...
  const { stats, monthlyData, serviceBreakdown, maxMonthlyTotal, maxServiceCount } =
    useDashboardStats(bookings);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (user.businessId) {
        ...
        // businessId is missing from the stored session (e.g. stale login before this fix).
        // Recover gracefully by fetching the full profile from /me.
        console.warn("[BusinessDashboard] No businessId in stored session, fetching from /me ...");
        fetchProfile()
          .then((profile) => {
            if (profile.businessId) {
              console.log("[BusinessDashboard] Recovered businessId from /me:", profile.businessId);
              setBusinessId(String(profile.businessId));
              // Patch the stored user so future loads skip this fallback
              const patched = {
                ...user,
                businessId: String(profile.businessId),
                businessName: profile.businessName ?? user.businessName,
                businessCategoryName: profile.businessCategoryName ?? user.businessCategoryName,
                hasBusiness: true,
              };
              localStorage.setItem("user", JSON.stringify(patched));
            } else {
              console.warn("[BusinessDashboard] /me returned no businessId either");
              setError(dashboardT(locale, "dashboard_error_no_business"));
              setLoading(false);
            }
          })
          .catch((err) => {
            console.error("[BusinessDashboard] Failed to fetch profile from /me:", err);
            setError(dashboardT(locale, "dashboard_error_no_business"));
            setLoading(false);
          });
      }
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      const token =
        localStorage.getItem("accessToken") || localStorage.getItem("token") || undefined;
      ...
      console.log("[BusinessDashboard] Fetching data with params:", {
        businessId,
        from,
        to,
        hasToken: !!token,
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-background to-slate-100/50 dark:from-slate-950 dark:via-background dark:to-slate-900/50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400">
            {dashboardT(locale, "dashboard_loading")}
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-background to-slate-100/50 dark:from-slate-950 dark:via-background dark:to-slate-900/50 flex items-center justify-center p-6">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">
            {dashboardT(locale, "dashboard_error_title")}
          </h2>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {dashboardT(locale, "dashboard_total_bookings")}
            </p>
          </button>
          ...
          >
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {dashboardT(locale, "dashboard_completed")}
            </p>
          </button>
          ...
          >
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {dashboardT(locale, "dashboard_staff_members")}
            </p>
          </button>
          ...
          >
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {dashboardT(locale, "dashboard_services")}
            </p>
          </button>
        </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-white/80 text-sm font-medium uppercase tracking-wider">
                    {dashboardT(locale, "dashboard_total_revenue")}
                  </p>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-sm text-white/90">
                      {dashboardT(locale, "dashboard_revenue_from_completed", {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            ...
            <h3 className="font-bold text-slate-800 dark:text-white text-lg mb-1">
              {dashboardT(locale, "dashboard_manage_bookings_title")}
            </h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
              {dashboardT(locale, "dashboard_manage_bookings_desc")}
            </p>
          ...
          >
            ...
            <h3 className="font-bold text-slate-800 dark:text-white text-lg mb-1">
              {dashboardT(locale, "dashboard_manage_staff_title")}
            </h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
              {dashboardT(locale, "dashboard_manage_staff_desc")}
            </p>
          ...
          >
            ...
            <h3 className="font-bold text-slate-800 dark:text-white text-lg mb-1">
              {dashboardT(locale, "dashboard_manage_services_title")}
            </h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
              {dashboardT(locale, "dashboard_manage_services_desc")}
            </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-800 dark:text-white text-lg">
                    {dashboardT(locale, "dashboard_recent_bookings_title")}
                  </h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    {dashboardT(locale, "dashboard_recent_bookings_desc")}
                  </p>
                </div>
              </div>
              <Button
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                      ...
                      <span dir="ltr">
                        {new Date(booking.date).toLocaleDateString(DATE_LOCALE_MAP[locale])}
                      </span>
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${getBookingStatusClass(booking.status)}`}
                    >
        ...
        {recentBookings.length === 0 && !loading && (
          <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border border-slate-200/60 dark:border-slate-700/60 rounded-2xl p-12 text-center shadow-lg">
            ...
            <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">
              {dashboardT(locale, "dashboard_no_bookings_title")}
            </h3>
