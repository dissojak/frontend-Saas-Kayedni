...
const API_BASE_URL = normalizeBaseUrl(
  process.env.NEXT_PUBLIC_AUTH_URL || "http://localhost:8088/api/v1/auth",
);
...
export interface TwoFactorSendCodePayload {
  ...
  /** Why the code is being sent — used by backend for context-aware email messages.
   *  Values: 'SETUP' | 'DISABLE_METHOD' | 'BACKUP_CODES' */
  context?: string;
}
 ...
 */
export async function validateInviteKeyAPI(
  inviteKey: string,
): Promise<{ status: number; data?: any }> {
  try {
...
export async function sendTwoFactorLoginCodeAPI(
  payload: TwoFactorLoginSendCodePayload,
...
export async function sendTwoFactorSetupCodeAPI(
  accessToken: string,
