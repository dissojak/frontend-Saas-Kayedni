...
export async function PATCH(
  ...
  try {
    ...
    const upstream = await fetch(`${TRACKING_SERVICE_URL}/api/session/${sessionId}/activity`, {
      method: "PATCH",
      headers: {
    ...
    return NextResponse.json({ message: "Failed to proxy session activity" }, { status: 502 });
  }
}
